...
4. 读取 `docs/CURRENT_WORKFLOW.md` 和当前 `docs/phases` 文档。
5. 执行 `git status --short --branch`、`git diff`、`git diff --cached`。
6. 只读取本次任务直接相关的架构、协议、硬件、源码和最近 worklog。

`PROJECT_STATUS.md` 是当前事实的权威摘要；`CURRENT_HANDOFF.md` 是最近工作断点；worklog 记录
过程和证据。不得依赖聊天记忆替代这些文件。
