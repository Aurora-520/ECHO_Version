...
5. 读取 `docs/CURRENT_WORKFLOW.md` 和当前 `docs/phases` 文档。
6. 读取 `docs/CAPABILITY_CATALOG.md` 和本任务要使用模块的能力文档。
7. 读取 `docs/debuglogs/README.md` 和当前阶段/模块相关的最近调试日志。
8. 读取 `docs/learning/README.md` 和本任务相关的学习日志。
9. 执行 `git status --short --branch`、`git diff`、`git diff --cached`。
10. 只读取本次任务直接相关的架构、协议、硬件、源码和最近 worklog。
