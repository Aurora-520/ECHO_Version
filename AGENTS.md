...
- MaixCAM Pro 官方资料和实机文件确认板载六轴 IMU；使用时仍必须通过独立 `ImuSource`，记录
  坐标轴、安装方向、采样率和标定。当前尚未完成实时 IMU 读取测试。
...
当前阶段是 V1“稳定采集与调试链”。V1.0 只读发现已完成，当前重点是相机唯一所有者、曝光、
时间戳、原图/状态抓取和有界录像，不是堆入具体 Mission。传统视觉和几何标定优先，跟踪其次，
神经网络模型第三。
