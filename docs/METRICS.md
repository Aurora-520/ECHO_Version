...
- 相机曝光、增益、白平衡、图像亮度、高亮/暗部裁剪、清晰度及其变化范围。
- nominal、boundary、adverse、negative、recovery 各切片的样本数和独立指标。
- 最差场景切片、最差连续失败长度、错误恢复时间和安全拒绝比例。
...
- 最差切片：预先定义的场景切片中表现最低的一项，不能在看完结果后删除或重新命名切片。

总体平均值不能替代各切片结果。所有数字必须来自机器可读摘要、日志或健康快照。
