...
- CameraSource 拥有曝光/增益/白平衡控制，并发布实际参数；算法不得绕过端口修改相机。
- 图像质量评估、算法检测和多帧估计分层，跟踪预测不得冒充当前帧真实检测。
- 参数更新通过配置服务校验并在帧/Pipeline 边界原子应用，保留版本与回退点。
- 失败帧保存、指标和调试服务是有界旁路，磁盘或网络故障不得阻塞主路径。
- Mission 只能选择已声明鲁棒性等级和适用范围的能力，不能静默放宽 valid/confidence/age 门禁。
